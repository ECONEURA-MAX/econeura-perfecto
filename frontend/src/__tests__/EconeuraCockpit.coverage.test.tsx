...
describe('EconeuraCockpit coverage-focused tests', () => {
  ...
  it.skip('invokeAgent returns body.output when fetch ok and token present (okr mapping)', async () => {
  ...
  it.skip('runAgent flow: non-ok response creates ERROR activity', async () => {
    const fetchMock = vi.fn(() =>
      Promise.resolve({ ok: false, json: () => Promise.resolve({}) } as any)
    );
    vi.stubGlobal('fetch', fetchMock);
  ...
  it.skip('openChatWithErrorSamples opens chat drawer with messages', async () => {
